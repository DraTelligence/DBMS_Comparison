param(
    ...
    [int]   $ColdRuns = 1,
    [int]   $HotRuns = 1,
    [string]$RestartContainerName = "opengauss",
...
$OutRoot = Join-Path $ProjectRoot "results\og\default"
...
function Invoke-Bench([string]$cacheMode, [int]$iter) {
    ...
    docker run --rm -e PGPASSWORD=$Pass `
        ...
        -lc "psql -X -v ON_ERROR_STOP=1 -h $DbHost -p $Port -U $User -d $Db -f /sql/bench_default.sql 2>&1 | tee /out/$run.txt"
}
