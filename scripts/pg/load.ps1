# pg_load_and_bench.ps1 — openGauss: rebuild -> import -> bench
$ErrorActionPreference = "Stop"
$DataDir   = "E:\workSpace\files\project\dbms_comparison\data\raw"
$CsvName   = "yellow_2019_07.csv"
$Schema    = "schema_all.sql"
$OutDir    = "E:\workSpace\files\project\dbms_comparison\results\pg"
...
$benchOut  = Join-Path $OutDir "pg_bench_$ts.txt"
$planOut   = Join-Path $OutDir "pg_explain_$ts.txt"
...
# 3) Import CSV (map empty string to NULL)
docker run --rm -e PGPASSWORD="$Pass" -v "${DataDir}:/data" -it postgres:latest `
