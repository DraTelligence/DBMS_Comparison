...
\echo === PLAN U1 (PARITY, ANALYZE, ROLLBACK) ===
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
UPDATE yellow_trips
...
\echo === PLAN U2 (PARITY, ANALYZE, ROLLBACK) ===
BEGIN;
EXPLAIN (ANALYZE, BUFFERS, TIMING)
UPDATE yellow_trips
...
\echo === PLAN U3-main (PARITY, NO-ANALYZE) ===
EXPLAIN (BUFFERS, SUMMARY)
UPDATE yellow_trips
   SET improvement_surcharge = ROUND(COALESCE(improvement_surcharge,0.00)+0.01,2);

\echo === PLAN U3-sandbox (PARITY, ANALYZE) ===
